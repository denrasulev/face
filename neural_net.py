...
experiment = Experiment(api_key="vk8Cbl1GfoLCBZPVWrBq17h1F",
                        project_name="Face Detection", workspace="denrasulev")
...
def train_model(tf_model, tf_criterion, tf_optimizer, tf_scheduler, tf_epochs):
    ...
    for ep in range(tf_epochs):
        ...
        for tf_mode in ['train', 'valid']:
            ...
            # Iterate over data
            for i, (inputs, labels) in enumerate(DATA_LOADER[tf_mode], 0):
                ...
                if tf_mode == 'train':
                    with torch.set_grad_enabled(True):
                        ...
                        total += labels.size(0)
                        correct += float((predictions == labels.data).sum())
